...
(function() {
    ...
    const QUESTIONS = [
        {
            id: 1,
            text: 'چند دستگاه قرار است به سیستم متصل شوند؟',
            options: [
                { 
                    text: '1 دستگاه', 
                ...
                { 
                    ...
                    reason: 'تعداد دستگاه‌ها بسیار زیاد است'
                }
            ]
        },
        {
            id: 2,
            text: 'داده‌ها چند مصرف‌کننده دارند؟',
            options: [
                { 
                    text: 'فقط یک سرویس', 
                    scores: { MQTT: 1, HTTP: 3 },
                ...
                { 
                    text: 'چند سرویس', 
                    scores: { MQTT: 3, HTTP: 1 },
                    reason: 'چندین مصرف‌کننده برای داده وجود دارد'
                },
                { 
                    text: 'تعداد مصرف‌کننده‌ها ممکن است تغییر کند', 
                    scores: { MQTT: 4, HTTP: 0 },
                    reason: 'مصرف‌کننده‌ها داینامیک هستند'
                }
            ]
        },
        {
            id: 3,
            text: 'مدل ارتباطی موردنیاز چیست؟',
            options: [
                { 
                    text: 'Request / Response', 
                    scores: { MQTT: 0, HTTP: 4 },
                    reason: 'نیاز به مدل Request/Response دارید'
                },
                { 
                    text: 'Publish / Subscribe', 
                    scores: { MQTT: 4, HTTP: 0 },
                    reason: 'نیاز به مدل Publish/Subscribe دارید'
                },
                { 
        ...
        {
            id: 4,
            text: 'دستگاه‌ها باید ارتباط دائمی داشته باشند؟',
            options: [
                { 
                    text: 'بله', 
                    scores: { MQTT: 3, HTTP: 1 },
                    reason: 'ارتباط دائمی مورد نیاز است'
                },
                { 
                    text: 'خیر', 
                    scores: { MQTT: 1, HTTP: 3 },
        ...
        {
            id: 5,
            text: 'آیا Backend شما REST API دارد؟',
            options: [
                { 
                    text: 'بله', 
                    scores: { MQTT: 1, HTTP: 3 },
    ...
    function getRecommendation() {
        ...
        if (totalScore === 0) {
            return {
                ...
                alternative: 'با پاسخ دادن به سؤالات بیشتر می‌توانید پیشنهاد دقیق‌تری دریافت کنید.'
            };
        }
        ...
        if (mqttPercent > httpPercent) {
            ...
            if (reasons.length < 2) {
                if (AppState.scores.MQTT > AppState.scores.HTTP * 1.5) {
                    reasons.push('امتیاز MQTT به طور قابل توجهی بالاتر است');
                }
                reasons.push('معماری Publish/Subscribe مناسب‌تر است');
            }
            
            alternative = 'HTTP هم می‌تواند مناسب باشد اگر نیاز به یکپارچگی ساده با REST API دارید یا تعداد مصرف‌کننده‌ها محدود است.';
            ...
            if (reasons.length < 2) {
                if (AppState.scores.HTTP > AppState.scores.MQTT * 1.5) {
                    reasons.push('امتیاز HTTP به طور قابل توجهی بالاتر است');
                }
                reasons.push('مدل Request/Response مناسب‌تر است');
            }
            
            alternative = 'MQTT هم می‌تواند مناسب باشد اگر در آینده نیاز به اضافه کردن مصرف‌کننده‌های بیشتر دارید یا می‌خواهید معماری Event-driven داشته باشید.';
            ...
            matchPercent = 50;
            reasons = [
                'نیازهای پروژه شما با هر دو پروتکل سازگار است',
                'انتخاب نهایی به سایر ملاحظات فنی بستگی دارد'
            ];
            alternative = 'در این حالت، توصیه می‌کنیم سایر عوامل مانند زیرساخت موجود، تخصص تیم، و ملاحظات بلندمدت را در نظر بگیرید.';
        }
